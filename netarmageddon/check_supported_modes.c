 ...
 * install
 * sudo apt install -y libnl-3-dev libnl-genl-3-dev linux-headers-$(uname -r)
 * Compile:
 * gcc check_supported_modes.c -o check_supported_modes -lnl-genl-3 -lnl-3 -I/usr/include/libnl3 -g -Wall -O3
*/
...
int retrieve_iftypes_callback(struct nl_msg* msg, void* arg)
{
    ...
    if (tb_msg[NL80211_ATTR_IFNAME]) {
        snprintf(current->dev.name, IFNAMSIZ, "%s",
